...
- **`NavKey`s stay out of screen composables.** Screens expose intent callbacks (`onTransfersClick: () -> Unit`); the destination is the only place that constructs a key and calls `navigationHandler.navigate(...)`. A screen that builds its own `NavKey` is coupled to the navigation graph and can't be reused or previewed.
- **Never pass a `NavigationHandler` down to child composables**, and don't wrap one in a `CompositionLocal` — emit through `NavigationEventQueue` (`:core:navigation-contract`, `…/queue/NavigationEventQueue.kt`) instead. Caveat: the queue routes to the *main* single-activity host, so a screen hosted inside its own scaffold (e.g. the PDF viewer inside `ChatActivity`) must navigate within that host rather than via the queue.
- **`entry<>` metadata is bound statically per NavKey *type*** and evaluated at registration, not at navigation time. A field on the key (`forceDarkTheme = true`) cannot vary it per instance — declare a separate NavKey type for each metadata variant. This matters for anything the scene wraps outside the content lambda, such as a bottom sheet's container background.
- **Don't put large payloads in a `NavKey`.** Keys are Parcelable and cross a Binder transaction, so a `LongArray` of thousands of node handles risks `TransactionTooLargeException`. Use the existing RouteLauncher + launch-source-holder pattern (see `Nav3VideoPlayerRouteLauncher`).
- Legacy bridging goes through `AppNavigator`/`MegaNavigator` plus the feature-flag gate, so the legacy/Nav3 decision stays in one place — not ad-hoc `startActivity` calls at each site.
