 ...
 * spec distinguishes free from Pro users, so every scenario has a free and a Pro variant.
 */
internal class QuotaWarningEventMapper @Inject constructor() {
    ...
    operator fun invoke(
        ...
        QuotaWarningScenario.TransferAlmostUsed -> if (isProUser) {
            QuotaWarningEvents(
        ...
        QuotaWarningScenario.TransferAllUsed -> if (isProUser) {
            QuotaWarningEvents(
